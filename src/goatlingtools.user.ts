...
class Logger {
  ...
  private log(level: number, ...msg: any) {
    if (level >= this.logLevel)
      console.log(
        `[GoatTools:${Logger.LOG_REPR[level]}] ${this.__repr()} `, ...msg
      )
  }

  logInfo(msg: any) { return this.log(Logger.LOG_INFO, msg) }
  logWarn(msg: any) { return this.log(Logger.LOG_WARNING, msg) }
  logError(msg: any) { return this.log(Logger.LOG_ERROR, msg) }
  logDebug(msg: any) { return this.log(Logger.LOG_DEBUG, msg) }
  logVerbose(msg: any) { return this.log(Logger.LOG_VERBOSE, msg) }
...
class Style {
  ...
  static set background(color: string) { this.settings.background = color }
  static set primary(color: string) { this.settings.primary = color }
...
class User extends Logger {
  ...
  constructor(
    ...
    super(() => { return this.name ? `User[${String(this.name)}]` : "User[GuestUser]" })
    this.uuid = uuid
    this.name = name
  }
  ...
  async fetchGoatlings() {
    if (!this.__goatlings) {
      this.__goatlings = JSON.parse(localStorage.getItem(`${this.uuid}_goatlings`) ?? "{}")
    }
  ...
  async updateGoatlings(): Promise<void> {
    ...
    if (getUri() == "/mypets") {
      ...
      $.ajax({
        url: `${PAGE}/mypets`, async: true, success: (data) => {
          this.__goatlings = this.updateGoatlingsActual($(data).find("div.mypets-pet"))
          localStorage.setItem(`${this.uuid}_goatlings`, JSON.stringify(this.__goatlings))
          this.logDebug("updateGoatlings() Finished update")
        }
      })
  ...
  private async checkNeedsUpdate(what: string): Promise<boolean> {
    ...
    let last = Math.floor(parseSepInt(localStorage.getItem(`${this.uuid}_${what}_last_update`) ?? "0"))
...
class Mod extends Logger {
  ...
  get user() { return Mod.user }

  constructor(name: string) {
    super(() => { return `Mod[${name}]` })
    this.name = name
    this.enabled = true
    this.onActivate = async () => { return true }
    this.onPreload = () => { return true }
  }
...
Mod.create("hagglePrice", (mod) => {
  ...
  mod.onActivate = async () => {
    ...
    $("#content > center > form").before(`
      <div class="gt-haggle-price">Haggle price: <b>${price * 0.80}</b></div><br>
    `)
...
$(async () => {
  ...
  if (typeof csrf === "string") {
    ...
    const currency = $("div#user-info-points")
      ?.text()
      ?.replace(/[\s]+/g, ";").split(";")
      ?? []
    ...
    user.active = (/Welcome\s+[^\s]+\s+-\s+Your\s+active\s+Goatling\s+is\s+([^\s]+)/)
      ?.exec($("div#user-info").text())
      ?.[1] ?? null
